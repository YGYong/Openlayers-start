...
1. 使用 `↑` 向上和 `↓` 向下箭头键调整望远镜大小。
2. 利用`postrender`事件侦听器在指针位置周围的圆圈中对图像进行过采样。此事件的侦听器可以访问 Canvas 上下文，并可以作图像数据。
3. 注意高德底图跨域配置，`crossOrigin: 'anonymous'`。
