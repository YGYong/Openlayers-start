...
1. 鼠标靠近自动吸附到边界线
2. 使用 `traceSource` 选项从一个源跟踪要素并将其添加到另一个源，第一次单击目标要素的边缘将开始追踪。第二次单击边缘将停止追踪。
...
const drawOptions = {
  ...
  trace: true, // 启用追踪
  traceSource: baseVectorSource, // 追踪数据源
};
```
...
<template>
  <div class="map-container">
    ...
    <div class="controls">
      ...
      <button @click="clearDrawings" class="clear-btn">清除绘制</button>
    </div>
  </div>
</template>
...
<video controls>
  <source src="./assets/35_追踪要素.mp4" type="video/mp4" />
  您的浏览器不支持HTML5视频标签。
</video>
