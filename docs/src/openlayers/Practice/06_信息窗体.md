...
<template>
  <div class="ol-tutorial-demo-container">
    ...
    <div v-if="pointPopup" class="popup" :style="pointPopupStyle">
      ...
      <button class="close-btn" @click="removePoint(pointPopup.id)">
        删除
      </button>
...
function centerBeijing() {
  map.value.getView().setCenter(fromLonLat([116.4, 39.9]));
...
onMounted(() => {
  ...
  rotation.value = (
    (map.value.getView().getRotation() * 180) /
    Math.PI
  ).toFixed(1);
  ...
  dragBox.value.on("boxend", (e) => {
    ...
    boxInfo.value = {
      sw: `${sw[0].toFixed(6)}, ${sw[1].toFixed(6)}`,
      ne: `${ne[0].toFixed(6)}, ${ne[1].toFixed(6)}`,
    };
    boxPopupStyle.value = {
      left: e.target.box_.endPixel_[0] + 20 + "px",
      top: e.target.box_.endPixel_[1] + 20 + "px",
    };
  });
  // 右键菜单
...
<video controls>
  <source src="./assets/06信息窗体.mp4" type="video/mp4" />
  您的浏览器不支持HTML5视频标签。
</video>
