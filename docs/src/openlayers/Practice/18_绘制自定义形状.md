...
1. 绘制自定义形状需要使用 `Draw` 交互，同时设置 `geometryFunction` 来自定义形状的几何函数。
2. 自定义形状的几何函数需要接收绘制的坐标数组和几何对象作为参数，返回一个新的几何对象。
...
<template>
  <div class="map-container">
    ...
    <div class="controls">
      ...
      <button
        @click="clearFeatures"
        :disabled="!vectorSource || vectorSource.getFeatures().length === 0"
      >
        清除所有五角星
      </button>
    </div>
