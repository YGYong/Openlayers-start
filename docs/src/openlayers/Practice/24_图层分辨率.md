...
1. 使用分辨率控制
   ...
   - 详细图层：`minResolution: 0.0001, maxResolution: 0.001`
   - 概览图层：`minResolution: 0.001, maxResolution: 0.1`
   - OpenLayers 会根据当前分辨率自动显示/隐藏图层

2. 注意`EPSG:4326分辨率值`和`EPSG:3857分辨率值`不同
3. 使用`layer.setVisible`方法手动控制图层显隐是一样的效果
...
const isDetailLayerActive = computed(() => {
  return (
    currentResolution.value >= DETAIL_MIN_RESOLUTION &&
...
const isOverviewLayerActive = computed(() => {
  return (
    currentResolution.value >= OVERVIEW_MIN_RESOLUTION &&
...
const zoomToDetail = () => {
  if (map) {
    ...
    const targetResolution =
      (DETAIL_MIN_RESOLUTION + DETAIL_MAX_RESOLUTION) / 2;
    const view = map.getView();
...
const zoomToOverview = () => {
  if (map) {
    // 设置到概览图层分辨率范围内
    const targetResolution =
      (OVERVIEW_MIN_RESOLUTION + OVERVIEW_MAX_RESOLUTION) / 2;
    const view = map.getView();
...
<video controls>
  <source src="./assets/24_图层分辨率.mp4" type="video/mp4" />
  您的浏览器不支持HTML5视频标签。
</video>
