...
1. 注意你的图片大小，这里图片是 128\*64 的，所以`icon-width`和`icon-height`要写 128 和 64
2. `icon-offset`是图标偏移量，对`type`进行匹配，**如果都没有匹配到，就取最后一个值，这里是[96, 32]，类似于 switch 语句**
...
![精灵图](./assets/37_原图.png)

### 效果图

![精灵图](./assets/37_精灵图.png)
