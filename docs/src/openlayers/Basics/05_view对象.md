...
View（视图）是 OpenLayers 中控制地图显示范围和状态的核心组件
...
直接设置分辨率，一般直接设置`zoom`即可，除非需要精确控制显示比例时使用。关于`resolution`相关的，更多介绍可查阅文档，[View](https://openlayers.org/en/latest/apidoc/module-ol_View-View.html)。
...
- rotation：目标旋转
- duration：动画时长（毫秒）
- easing：缓动函数 参考 [常用 api](./99_常用api.md#easing)
...
`fit(geometryOrExtent, options)`调整视图以适配指定的几何图形或范围
