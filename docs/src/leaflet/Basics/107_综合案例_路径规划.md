...
这里调用了**OpenStreetMap 的 OSRM (Open Source Routing Machine) 公共服务**。OSRM 的公共服务是用于演示和测试的，不适合高并发或生产环境使用，因为它有严格的速率限制。

> 注意：
>
> 1. 需要使用 OSM 底图，高德底图存在坐标偏差。
> 2. OSM 底图需要挂一个梯子访问
...
<video controls>
  <source src="./imgs/07_路径规划.mp4" type="video/mp4" />
  您的浏览器不支持HTML5视频标签。
</video>
